...
export const HeroSection: React.FC<{ data: WeddingData }> = ({ data }) => (
  <section className="relative h-screen min-h-[600px] w-full overflow-hidden flex flex-col items-center justify-center">
    <div className="absolute inset-0">
      ...
      <img src={IMAGES.coupleHero} alt="Couple Hero" className="w-full h-full object-cover object-top opacity-90" />
  ...
  <section className="py-16 px-4 max-w-5xl mx-auto bg-white/50 rounded-3xl my-8 shadow-sm border border-stone-100 relative overflow-hidden">
    <AnimatedSection className="text-center">
      <div className="border-l-2 border-sage-400 pl-6 inline-block text-left mb-12">
        <h2 className="font-serif text-3xl md:text-4xl mb-2 text-stone-800">Thư Mời Cưới</h2>
        <p className="text-stone-500 font-sans tracking-wide text-sm">Trân trọng kính mời quý khách</p>
      </div>

      <div className="grid md:grid-cols-2 gap-8 relative z-10">
        <div className="text-center md:text-right space-y-1">
            <h3 className="font-bold uppercase text-sage-800 text-xs tracking-widest mb-3">Nhà Gái</h3>
            <p className="font-serif text-lg md:text-xl text-stone-700">{data.bride.parents.father}</p>
            <p className="font-serif text-lg md:text-xl text-stone-700">{data.bride.parents.mother}</p>
        </div>
        ...
        <div className="text-center md:text-left space-y-1">
            <h3 className="font-bold uppercase text-sage-800 text-xs tracking-widest mb-3">Nhà Trai</h3>
            <p className="font-serif text-lg md:text-xl text-stone-700">{data.groom.parents.father}</p>
            <p className="font-serif text-lg md:text-xl text-stone-700">{data.groom.parents.mother}</p>
        </div>
      </div>
    </AnimatedSection>
  </section>
  ...
  <section className="py-16 px-4">
    <div className="max-w-6xl mx-auto">
        <AnimatedSection className="text-center mb-10">
            <h2 className="font-serif text-3xl md:text-4xl text-stone-800 mb-8">Trân Trọng Kính Mời</h2>
        </AnimatedSection>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 px-4">
            <AnimatedSection className="delay-100 overflow-hidden bg-stone-200 p-2 shadow-md rotate-[-2deg]">
                 <img src={IMAGES.gallery1} className="w-full h-auto aspect-[3/4] object-cover object-center grayscale hover:grayscale-0 transition-all duration-500" alt="Moment 1" />
            </AnimatedSection>
            <AnimatedSection className="delay-200 overflow-hidden bg-stone-200 p-2 shadow-md z-10 scale-105">
                 <img src={IMAGES.couplePortrait} className="w-full h-auto aspect-[3/4] object-cover object-top" alt="Moment 2" />
            </AnimatedSection>
            <AnimatedSection className="delay-300 overflow-hidden bg-stone-200 p-2 shadow-md rotate-[2deg]">
                 <img src={IMAGES.gallery2} className="w-full h-auto aspect-[3/4] object-cover object-center grayscale hover:grayscale-0 transition-all duration-500" alt="Moment 3" />
            </AnimatedSection>
        </div>
        ...
        <AnimatedSection>
            <div className="bg-[#eff2ef] rounded-3xl p-8 text-center max-w-2xl mx-auto shadow-inner border border-white">
                <h3 className="font-bold uppercase tracking-widest text-base md:text-lg mb-4 text-stone-800">Tiệc Mừng Lễ Vu Quy</h3>
                ...
                <h4 className="text-base font-semibold text-stone-800">Tại {data.location.venue}</h4>
            </div>
        </AnimatedSection>
    </div>
  </section>
    ...
    return (
        <section className="py-12 px-4 bg-white">
            <AnimatedSection className="max-w-md mx-auto text-center">
                <h2 className="font-serif text-2xl uppercase tracking-widest mb-2">Save The Date</h2>
                <p className="font-serif italic text-lg mb-6">Tháng {date.month} - {date.year}</p>

                <div className="bg-[#f4f7f4] p-4 rounded-lg shadow-sm">
                    <div className="grid grid-cols-7 gap-2 mb-2">
                        {['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'].map((d, i) => (
                             <div key={i} className={`font-bold text-sm text-stone-600 ${i===0 || i===6 ? 'text-sage-600' : ''}`}>{d}</div>
                        ))}
                    </div>
                    <div className="w-full h-px bg-stone-300 mb-2"></div>
                    {weeks.map((week, i) => (
                        <div key={i} className="grid grid-cols-7 gap-2 mb-2 text-stone-600 font-sans text-sm">
                            {week.map((d, j) => {
                                const isEventDay = d === date.day;
                                return (
                                    <div key={j} className="flex items-center justify-center relative h-8">
                                        {d && (
                                            <>
                                              {isEventDay && (
                                                  <div className="absolute inset-0 flex items-center justify-center">
                                                      <svg className="w-8 h-8 text-sage-600 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <section className="py-16 px-4 bg-[#e3ebe3]/30">
        <AnimatedSection className="max-w-2xl mx-auto text-center space-y-4">
            ...
            <div className="space-y-1">
                <h3 className="font-bold text-lg uppercase text-sage-800">{location.venue}</h3>
                <p className="text-stone-600 max-w-xs mx-auto text-base">{location.address}</p>
            </div>
...
export const GallerySection: React.FC = () => (
    <section className="py-16 px-4">
        <AnimatedSection className="max-w-6xl mx-auto">
             <h2 className="font-script text-5xl text-center text-sage-700 mb-8">Album Hình Cưới</h2>
             ...
             <div className="columns-2 md:columns-4 gap-4 space-y-4">
                 ...
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.coupleHero} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 1" />
                 </div>
                 
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.gallery3} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 2" />
                 </div>
                 
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.gallery4} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 3" />
                 </div>
                 
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.gallery5} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 4" />
                 </div>
                 
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.gallery2} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 5" />
                 </div>
                 
                 <div className="break-inside-avoid rounded-lg overflow-hidden mb-4">
                     <img src={IMAGES.gallery1} className="w-full h-auto object-cover hover:scale-105 transition-transform duration-700" alt="Gallery 6" />
                 </div>
...
export const FooterSection: React.FC<FooterSectionProps> = ({ onClose }) => (
    <section className="relative h-[80vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
            ...
            <img src={IMAGES.footer} alt="Thank you" className="w-full h-full object-cover object-top" />
        ...
        <AnimatedSection className="relative z-10 text-center text-white px-4">
             ...
             <p className="font-serif text-lg md:text-xl italic opacity-90">Rất hân hạnh được đón tiếp!</p>
